# Carnitify - A BeamNG Mod Manager
A mod manager for BeamNG.drive with multiple features. (all of mods will show v1.0, i will not implement auto-update mods because some of you and me use paid mods or mods that aren't in the official beamng site, so i will not allow piracy, it will remain like this)

[![Download Latest Release](https://img.shields.io/badge/Download-Latest_Release-white?style=for-the-badge&logo=github)](https://github.com/CazymirTM/Carnitify/releases/latest)

## ⚙️ Features
- Auto-Mod folder scanning
- Backup current mods
- Restore Backup
- Refresh list
- Open mods folder
- Enable/Disable/Delete a specific mod
- Mod details
- Auto-check for updates on startup (in Settings menu)
- Custom Mods Folder/Path (in Settings menu)
- UI Theme: **Dark**, **Light** (in Settings menu)

![GUI of the program](CARNITIFYUI.png)
